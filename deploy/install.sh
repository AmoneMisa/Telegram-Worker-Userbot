#!/usr/bin/env bash
# One-time server bootstrap for the Docker Compose deployment.

set -euo pipefail

APP_DIR="${APP_DIR:-/opt/tg-worker}"
BRANCH="${BRANCH:-master}"
REPO="${REPO:-https://github.com/AmoneMisa/Telegram-Worker-Userbot.git}"

log() { printf '[install] %s\n' "$*"; }
die() { printf '[install] ERROR: %s\n' "$*" >&2; exit 1; }

[ "$(id -u)" -eq 0 ] || die "run as root (sudo)"
for bin in git docker curl; do
  command -v "$bin" >/dev/null 2>&1 || die "$bin is not installed"
done
docker compose version >/dev/null 2>&1 || die "Docker Compose v2 is required"

if [ -d "$APP_DIR/.git" ]; then
  log "reusing existing checkout at $APP_DIR"
else
  log "cloning $REPO into $APP_DIR"
  mkdir -p "$(dirname "$APP_DIR")"
  git clone --branch "$BRANCH" "$REPO" "$APP_DIR"
fi
cd "$APP_DIR"

# Preserve credentials from the old systemd deployment when upgrading.
if [ ! -f .env ] && [ -f worker.env ]; then
  mv worker.env .env
  log "migrated worker.env to .env"
fi
if [ ! -f .env ]; then
  cp sample.env .env
  log "created $APP_DIR/.env from sample.env"
fi
chmod 600 .env
mkdir -p photo-cache

# The runtime image uses the built-in Node user (uid 1000) and must be able to
# refresh the Telegram session and write cached photos through the bind mounts.
chown 1000:1000 .env photo-cache

if grep -Eq '^[[:space:]]*TG_SESSION[[:space:]]*=[[:space:]]*[^[:space:]]' .env; then
  log "building and starting tg-worker"
  docker compose up -d --build
  log "waiting for container health"
  for _ in $(seq 1 30); do
    if docker compose ps --format json | grep -q '"Health":"healthy"'; then
      log "tg-worker is healthy"
      exit 0
    fi
    sleep 2
  done
  docker compose logs --tail=100 tg-worker || true
  die "tg-worker did not become healthy"
fi

cat <<MSG

[install] Almost done — Telegram is not logged in yet.

  cd $APP_DIR
  docker compose build
  docker compose run --rm tg-worker npm run login
  docker compose up -d

The login writes TG_API_ID / TG_API_HASH / TG_SESSION into .env.
MSG
